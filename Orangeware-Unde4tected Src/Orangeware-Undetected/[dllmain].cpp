...
VOID mouse_event_(DWORD dwFlags, DWORD dx, DWORD dy, DWORD dwData, ULONG_PTR dwExtraInfo)
{
	...
	if (!doneonce)
	{
		if (!GetSyscallIndex(xorstrstr("win32u.dll"), xorstrstr("NtUserSendInput"), _NtUserSendInput))
			return;
		doneonce = true;
	}
...
void Menu(ImGuiWindow& window)
{
	...
	if (ShowMenu)
	{
		...
		if (ImGui::Begin(skCrypt(""), &ShowMenu, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoTitleBar))
		{
			...
			ImGui::GetWindowDrawList()->AddText(ImVec2(ImGui::GetWindowPos().x + 5, ImGui::GetWindowPos().y + 7), ImColor(200, 200, 200, 255), skCrypt("mcivan is gay and the son of masterpasta, dev: smartboy (jk hes gay)"));
			//ImGui::PopFont();
...
void Load()
{
	...
	LineOfSight = PatternScan_Module(ModuleBaseW, skCrypt("48 8B C4 48 89 58 20 55 56 57 41 54 41 55 41 56 41 57 48 8D 6C 24 ? 48 81 EC ? ? ? ? 0F 29 70 B8 0F 29 78 A8 44 0F 29 40 ? 48 8B 05 ? ? ? ? 48 33 C4 48 89 45 20 45 8A E9")); ////LineOfSight 48 8B F9 E8 ? ? ? ? 83 7E 08 00
